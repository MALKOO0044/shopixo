...
export type CjProductLike = {
  ...
  // Added fields for better product display (rating removed)
...
export async function getProductRating(_pid: string): Promise<{ rating: number | null; reviewCount: number }> {
  // Deprecated: CJ ratings are no longer imported or parsed
...
export async function getProductRatings(pids: string[]): Promise<Map<string, { rating: number | null; reviewCount: number }>> {
  // Deprecated: return an empty map with null ratings
  const m = new Map<string, { rating: number | null; reviewCount: number }>();
  for (const pid of pids) m.set(pid, { rating: null, reviewCount: 0 });
  return m;
}
...
function mergeListV2Fields(productData: any, match: any, source: string): void {
  ...
  // Rating fields ignored (legacy CJ ratings removed)
}
...
export function mapCjItemToProductLike(item: any): CjProductLike | null {
  ...
  // Ratings are not imported from CJ; ignore any CJ rating fields
  ...
  return {
    ...
    originArea,
    originCountryCode,
    // New fields for better product display (rating removed)
    price: minPrice,
    categoryName,
    sku,
    weight,
