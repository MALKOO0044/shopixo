...
export async function GET(req: Request) {
  return withRequestCounters(async () => {
    ...
    try {
      ...
      if (!guard.ok) {
        const r = NextResponse.json(withCallbackTelemetry({ ok: false, version: 'query-v4', error: guard.reason }), { status: 401, headers: { 'Cache-Control': 'no-store' } });
      ...
      if (!pid && !keyword) {
        const r = NextResponse.json(withCallbackTelemetry({ ok: false, error: 'Provide pid or keyword' }), { status: 400 });
        r.headers.set('x-request-id', log.requestId);
        return r;
      }
      ...
      if (pid) {
        ...
        for (let page = 1; page <= maxPages; page++) {
          ...
          const pageResult = await fetchCjProductPage(keyword, categoryId, page, pageSize);
          pagesFetched++;
      ...
      const r = NextResponse.json(
        withCallbackTelemetry({ ok: false, version: 'query-v4', error: e?.message || 'CJ query failed' }),
