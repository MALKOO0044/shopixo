...
type SearchStopReason =
  ...
  | 'media_filter_no_match'

  | 'candidate_exhausted'

  | 'fast_profile_shortfall'

  | 'shipping_shortfall'

  | 'batch_more_available'
...
async function handleSearch(req: Request, isPost: boolean) {
  ...
  try {
    ...
    try {
      const deletedFromSettings = await loadDiscoverDeletedPids()
    ...
    if (useOfflineDiscoverEngine) {
      ...
      if (!offlineResult.ok) {
        offlineFallbackReason = offlineResult.error || 'Offline discover failed'
        console.warn(`[Search&Price] Offline discover failed, falling back to CJ engine: ${offlineFallbackReason}`)
      } else {
      ...
      const offlineStopReason: SearchStopReason = quantityFulfilled
        ? 'fulfilled'
        : (isBatchMode && offlineResult.hasMore ? 'batch_more_available' : 'candidate_exhausted')
    ...
    let stopReason: SearchStopReason = quantityFulfilled ? 'fulfilled' : 'candidate_exhausted'
    ...
    if (!quantityFulfilled) {
      ...
      if (hitRateLimit) {
        ...
        stopReason = 'media_filter_no_match'
    ...
    const responseStopReason: SearchStopReason = quantityFulfilled
      ? 'fulfilled'
      : (isBatchMode && hasMoreToProcess && !hitRateLimit ? 'batch_more_available' : stopReason)
