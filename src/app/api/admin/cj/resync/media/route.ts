  ...
  return withRequestCounters(async () => {
    ...
    try {
      ...
      if (all) {
        ...
        productsToUpdate = (products || []).filter((p: any) => p.cj_product_id);
        ...
        return NextResponse.json(withCallbackTelemetry({ error: 'Provide productIds array or set all: true' }), { status: 400 });
      }

      if (productsToUpdate.length === 0) {
        return NextResponse.json(withCallbackTelemetry({ message: 'No products with CJ product IDs found', updated: 0 }));
      }
      ...
      return NextResponse.json(withCallbackTelemetry({ error: error.message || 'Internal server error' }), { status: 500 });
    }
  });
}
...
export async function GET(request: NextRequest) {
  return withRequestCounters(async () => {
    ...
    if (error) {
      return NextResponse.json(withCallbackTelemetry({ error: error.message }), { status: 500 });
    }
    ...
    const variantsWithImages = colorVariants.filter((v: any) => v.image_url);
    const variantsWithoutImages = colorVariants.filter((v: any) => !v.image_url);
    ...
    return NextResponse.json(withCallbackTelemetry({
      ...
      completionPercentage: colorVariants.length > 0
        ...
        : 100,
      products: stats?.map((p: any) => ({ id: p.id, title: p.title, cjPid: p.cj_product_id })),
    }));
  });
}
