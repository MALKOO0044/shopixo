  ...
  return withRequestCounters(async () => {
    ...
    try {
      ...
      if (!products || products.length === 0) {
        return NextResponse.json(withCallbackTelemetry({ ok: true, message: "No CJ products to sync", updated: 0, outOfStock: 0 }));
      }
      ...
      return NextResponse.json(withCallbackTelemetry({ ok: false, error: e?.message || "Server error" }), { status: 500 });
    }
  });
}
