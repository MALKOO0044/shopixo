...
import { fetchProductDetailsByPid, fetchProductDetailsByVariantVid } from '@/lib/cj/v2';
...
async function fetchProductByVariantSku(variantSku: string): Promise<any | null> {
  try {
    return await fetchProductDetailsByVariantVid(variantSku);
  } catch (e: any) {
...
export async function POST(req: Request) {
  return withRequestCounters(async () => {
    ...
    try {
      ...
      if (!supabase) {
        return NextResponse.json(
          withCallbackTelemetry({ ok: false, error: 'Database not configured' }),
          { status: 500 }
        );
      }
      ...
      if (all === true) {
        ...
        if (error) {
          return NextResponse.json(
            withCallbackTelemetry({ ok: false, error: error.message }),
            { status: 500 }
          );
        }
      ...
      return NextResponse.json(
        withCallbackTelemetry({ ok: false, error: 'Provide productId, productIds array, or all: true' }),
        { status: 400 }
      );
    } catch (e: any) {
      console.error('[Refresh Images] Error:', e);
      return NextResponse.json(
        withCallbackTelemetry({ ok: false, error: e?.message || 'Internal error' }),
        { status: 500 }
      );
    }
  });
  ...
  return withRequestCounters(async () => {
    ...
    try {
      ...
      const productStats = (products || []).map((p: any) => {
        ...
        else if (imgCount === 1) withOneImage++;
        else withMultipleImages++;
      ...
      return NextResponse.json(withCallbackTelemetry({ ok: true, ...stats }));
    } catch (e: any) {
