...
export default function ProductDiscoveryPage() {
  ...
  const searchProducts = async () => {
    ...
    const batchSize = 3; // Keep aligned with API default for safe timing
    const baseBatches = Math.ceil(quantity / batchSize);
    const maxBatches = Math.max(100, baseBatches * (media ? 5 : 3));
    ...
    try {
      ...
      while (hasMore && allProducts.length < quantity) {
        ...
        if (batchNumber >= maxBatches) {
          console.log(`Max batch limit reached (${maxBatches})`);
          break;
        }
      }
        ...
        body: JSON.stringify({
          ...
          products: selectedProducts.map(p => {
            ...
            const previewPayload = {
              ...
              inventoryVariants: p.inventoryVariants || [],
              listedNum: p.listedNum || 0,
              stock: p.stock ?? 0,
              totalVariants: p.totalVariants || 0,
              successfulVariants: p.successfulVariants || 0,
              displayedRating: p.displayedRating ?? null,
              ratingConfidence: p.ratingConfidence ?? null,
              categoryName: p.categoryName || '',
              productWeight: p.productWeight ?? null,
              packLength: p.packLength ?? null,
              packWidth: p.packWidth ?? null,
              packHeight: p.packHeight ?? null,
              material: p.material || '',
              productType: p.productType || '',
              sizeChartImages: p.sizeChartImages || [],
              processingTimeHours: p.processingTimeHours ?? null,
              deliveryTimeHours: p.deliveryTimeHours ?? null,
